...
def train_epoch(
	...
	for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
		...
		if (step % args.save_interval == 0 or step == iters - 1) and is_main_process():
			...
			ckp = (
				f'{args.save_dir}/'
				f'{args.save_weight}_{lm_config_student.hidden_size}{moe_suffix}.pth'
			)
...
if __name__ == '__main__':
	...
	parser.add_argument(
		'--max_seq_len', type=int, default=340, help='训练的最大截断长度（中文1token≈1.5~1.7字符）'
	)
	parser.add_argument(
	...
	if args.use_wandb and is_main_process():
		...
		wandb_run_name = (
			f'MiniMind-Distill-S{args.student_hidden_size}T'
			f'{args.teacher_hidden_size}-Epoch-'
			f'{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}'
		)
		wandb.init(
	...
	for epoch in range(start_epoch, args.epochs):
		...
		batch_sampler = SkipBatchSampler(
			train_sampler or indices,  # type: ignore
			args.batch_size,
			skip,
		)
		loader = DataLoader(
